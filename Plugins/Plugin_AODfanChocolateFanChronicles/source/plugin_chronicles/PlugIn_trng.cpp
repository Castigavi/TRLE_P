...
WORD SearchObjectSlot[QUANTITY_SEARCH_OBJECTS], Animation[QUANTITY_SEARCH_OBJECTS], Frame[QUANTITY_SEARCH_OBJECTS];
int SearchObjectIndex[SLOT_NUMBER_OBJECTS];
StrTestPosition Box[QUANTITY_SEARCH_OBJECTS];
StrMovePosition Position[QUANTITY_SEARCH_OBJECTS];
short TestPositionId[QUANTITY_SEARCH_OBJECTS];
bool OneShot[QUANTITY_SEARCH_OBJECTS];

// ************  Utilities section  ****************

DWORD MaximumDistance(StrItemTr4 *Source, StrItemTr4 *Target)
{
	DWORD DifferenceXAxis, DifferenceZAxis;
	int DifferenceYAxis;

	DifferenceXAxis = Source->CordX > Target->CordX ? Source->CordX - Target->CordX : Target->CordX - Source->CordX;
	DifferenceYAxis = Source->CordY > Target->CordY ? Source->CordY - Target->CordY : Target->CordY - Source->CordY;
	DifferenceZAxis = Source->CordZ > Target->CordZ ? Source->CordZ - Target->CordZ : Target->CordZ - Source->CordZ;
	if (DifferenceXAxis > DifferenceZAxis) {
		return DifferenceXAxis > (DWORD) DifferenceYAxis ? DifferenceXAxis : DifferenceYAxis;
	} else {
		return DifferenceZAxis > (DWORD) DifferenceYAxis ? DifferenceZAxis : DifferenceYAxis;
	}
}

bool AlignLara(StrItemTr4 *Object, StrMovePosition *Point)
{
	if (!MoveLaraPosition(Point, Object, Trng.pGlobTomb4->pAdr->pLara))
		return false;
	Trng.pGlobTomb4->TestAlignmentInProgress = false;
	*Trng.pGlobTomb4->pAdr->pObjectActive = INDEX_NONE;
	return true;
}
...
void cbInitLoadNewLevel(void)
{
	...
	for (Index = 0; Index < QUANTITY_SEARCH_OBJECTS; ++Index) {
		SearchObjectSlot[Index] = 0;
		Box[Index].Distance.MinX = 0;
		Box[Index].Distance.MaxX = 0;
		Box[Index].Distance.MinY = 0;
		Box[Index].Distance.MaxY = 0;
		Box[Index].Distance.MinZ = 0;
		Box[Index].Distance.MaxZ = 0;
		Box[Index].Orienting.OrientHMin = 0;
		Box[Index].Orienting.OrientHMax = 0;
		Box[Index].Orienting.OrientRMin = 0;
		Box[Index].Orienting.OrientRMax = 0;
		Box[Index].Orienting.OrientVMin = 0;
		Box[Index].Orienting.OrientVMax = 0;
		Position[Index].RelX = 0;
		Position[Index].RelY = 0;
		Position[Index].RelZ = 0;
		TestPositionId[Index] = 0;
		OneShot[Index] = false;
		Animation[Index] = ANIMATION_STANDING_PICKUP_FLOOR;
		Frame[Index] = FRAME_ADD_PICKUP;
	}
	for (Index = 0; Index < SLOT_NUMBER_OBJECTS; ++Index)
		SearchObjectIndex[Index] = INDEX_NONE;
}
...
void cbCustomizeMine(WORD CustomizeValue, int NumberOfItems, short *pItemArray)
{
	...
	Index = CustomizeValue >= CUST_SEARCH_OBJECT1 && CustomizeValue <= CUST_SEARCH_OBJECT16 ? CustomizeValue - 1 : INDEX_NONE;
	if (Index != INDEX_NONE) {
		if (NumberOfItems == ARGUMENT_COUNT_SEARCH_OBJECT) {
			TestPositionId[Index] = pItemArray[ARGUMENT1] > 0 && pItemArray[ARGUMENT1] < TEST_POSITION_ID_MAXIMUM ? pItemArray[ARGUMENT1] : 0;
			OneShot[Index] = pItemArray[ARGUMENT2] == 1;
			Animation[Index] = pItemArray[ARGUMENT3] >= 0 ? pItemArray[ARGUMENT3] : ANIMATION_STANDING_PICKUP_FLOOR;
			Frame[Index] = pItemArray[ARGUMENT4] >= 0 ? pItemArray[ARGUMENT4] : FRAME_ADD_PICKUP;
		} else {
			TestPositionId[Index] = 0;
			OneShot[Index] = false;
			Animation[Index] = ANIMATION_STANDING_PICKUP_FLOOR;
...
void cbAssignSlotMine(WORD Slot, WORD ObjType)
{
	...
	if (Slot >= SLOT_NUMBER_OBJECTS)
		Slot = 0;

	Index = ObjType >= OBJ_SEARCH_OBJECT1 && ObjType <= OBJ_SEARCH_OBJECT16 ? ObjType - 1 : INDEX_NONE;
	if (Index != INDEX_NONE)
		SearchObjectSlot[Index] = Slot;
}
...
bool TestSearchObjectConditions(StrItemTr4 *Object)
{
	StrItemTr4 *LaraObject;

	LaraObject = Trng.pGlobTomb4->pAdr->pLara;
	return *Trng.pGlobTomb4->pAdr->pInputGameCommands & CMD_ACTION
			&& (LaraObject->AnimationNow == ANIMATION_STAND_STILL_LINK || LaraObject->AnimationNow == ANIMATION_STAND_STILL_BREATH)
			&& *Trng.pGlobTomb4->pAdr->pFlagsLaraHands == FLH_ALMOST_FREE_HANDS;
}

void InitialiseSearchObject(short ItemIndex)
{
	StrItemTr4 *Object;

	Object = &Trng.pGlobTomb4->pAdr->pVetItems[ItemIndex];
	Object->StateIdCurrent = 0;
	Object->StateIdNext = 0;
	Object->AnimationNow = Trng.pGlobTomb4->pAdr->pVetSlot[Object->SlotID].IndexFirstAnim;
	Object->FrameNow = Trng.pGlobTomb4->pAdr->pVetAnimations[Object->AnimationNow].FrameStart;
	Object->FlagsMain = FITEM_CREATURE | FITEM_NOT_YET_ENABLED;
	Object->Objectbuttons |= MASK_ACTIVATION;
	AddActiveItem(ItemIndex);
}

void CollisionSearchObject(short ItemIndex, StrItemTr4 *pLara, StrCollisionLara *pLaraCollision)
{
	StrItemTr4 *Object;

	Object = &Trng.pGlobTomb4->pAdr->pVetItems[ItemIndex];
	if (!TriggerActive(Object)
			|| Object->AnimationNow == Trng.pGlobTomb4->pAdr->pVetSlot[Object->SlotID].IndexFirstAnim + 1
			|| MaximumDistance(pLara, Object) > DISTANCE_SECTOR
			|| !TestBoundCollide(Object, pLara, pLaraCollision->LaraSizeX)
			|| !TestCollision(Object, pLara))
		return;
	ItemPushLara(Object, pLara, pLaraCollision, false, 1);
}

void ControlSearchObject(short ItemIndex)
{
	StrItemTr4 *Object, *LaraObject;

	Object = &Trng.pGlobTomb4->pAdr->pVetItems[ItemIndex];
	AnimateItem(Object);
	if (Object->AnimationNow != Trng.pGlobTomb4->pAdr->pVetSlot[Object->SlotID].IndexFirstAnim + 1) {
		if (!Object->Reserved_34) {
			LaraObject = Trng.pGlobTomb4->pAdr->pLara;
			if (Trng.pGlobTomb4->TestAlignmentInProgress && *Trng.pGlobTomb4->pAdr->pObjectActive == ItemIndex) {
				if (*Trng.pGlobTomb4->pAdr->pFlagsLara & FL_SELF_ALIGNMENT) {
					if (!AlignLara(Object, &Position[SearchObjectIndex[Object->SlotID]]))
						return;
				} else {
					Trng.pGlobTomb4->TestAlignmentInProgress = false;
					*Trng.pGlobTomb4->pAdr->pObjectActive = INDEX_NONE;
				...
				if (!TestSearchObjectConditions(Object) || !TestLaraPosition(&Box[SearchObjectIndex[Object->SlotID]], Object, LaraObject))
					return;
				Trng.pGlobTomb4->TestAlignmentInProgress = true;
				*Trng.pGlobTomb4->pAdr->pObjectActive = ItemIndex;
				if (!AlignLara(Object, &Position[SearchObjectIndex[Object->SlotID]]))
					return;
			}
			LaraObject->AnimationNow = Animation[SearchObjectIndex[Object->SlotID]];
			LaraObject->FrameNow = Trng.pGlobTomb4->pAdr->pVetAnimations[LaraObject->AnimationNow].FrameStart;
			LaraObject->StateIdCurrent = Trng.pGlobTomb4->pAdr->pVetAnimations[LaraObject->AnimationNow].StateId;
			LaraObject->StateIdNext = Trng.pGlobTomb4->pAdr->pVetAnimations[LaraObject->AnimationNow].StateId;
			Object->AnimationNow = Trng.pGlobTomb4->pAdr->pVetSlot[Object->SlotID].IndexFirstAnim + 1;
			Object->FrameNow = Trng.pGlobTomb4->pAdr->pVetAnimations[Object->AnimationNow].FrameStart;
			Object->StateIdCurrent = Trng.pGlobTomb4->pAdr->pVetAnimations[Object->AnimationNow].StateId;
			Object->StateIdNext = Trng.pGlobTomb4->pAdr->pVetAnimations[Object->AnimationNow].StateId;
			*Trng.pGlobTomb4->pAdr->pFlagsLara &= ~FL_SELF_ALIGNMENT;
			Object->Reserved_34 = OneShot[SearchObjectIndex[Object->SlotID]];
		}
	} else {
		if (!Object->Reserved_36
				&& Object->FrameNow == Trng.pGlobTomb4->pAdr->pVetAnimations[Object->AnimationNow].FrameStart + Frame[SearchObjectIndex[Object->SlotID]]) {
			if (Object->OcbCode)
				PerformTriggerGroup(Object->OcbCode);
			Object->Reserved_36 = true;
		}
	}
}

void InitSearchObject(void)
{
	StrSlot *Slot;
	int Index, Command;

	for (Index = 0; Index < QUANTITY_SEARCH_OBJECTS; ++Index) {
		if (SearchObjectSlot[Index]) {
			if (TestPositionId[Index]) {
				for (Command = 0; Command < Trng.pGlobTomb4->BaseTestPosition.TotTestPositions; ++Command) {
					if (Trng.pGlobTomb4->BaseTestPosition.VetTestPosition[Command].IdTestPosition == TestPositionId[Index]) {
						Slot = &Trng.pGlobTomb4->pAdr->pVetSlot[SearchObjectSlot[Index]];
						if (Slot->Flags & FSLOT_PRESENT) {
							Slot->pProcInitialise = InitialiseSearchObject;
							Slot->pProcCollision = CollisionSearchObject;
							Slot->pProcControl = ControlSearchObject;
							Slot->Flags = FSLOT_SAVE_ALL_DATA | FSLOT_AFFECT_LARA_AT_CONTACT | FSLOT_SFX_LOCAL_SOUND | FSLOT_USE_COLLISION_BOX;
							SearchObjectIndex[SearchObjectSlot[Index]] = Index;
							Box[Index] = Trng.pGlobTomb4->BaseTestPosition.VetTestPosition[Command].DatiPosition;
							Box[Index].Orienting.OrientHMax = -Box[Index].Orienting.OrientHMin;
							Box[Index].Orienting.OrientRMax = -Box[Index].Orienting.OrientRMin;
							Box[Index].Orienting.OrientVMax = -Box[Index].Orienting.OrientVMin;
							Position[Index].RelX = (Box[Index].Distance.MinX + Box[Index].Distance.MaxX) / 2;
							Position[Index].RelY = (Box[Index].Distance.MinY + Box[Index].Distance.MaxY) / 2;
							Position[Index].RelZ = (Box[Index].Distance.MinZ + Box[Index].Distance.MaxZ) / 2;
							break;
						}
					}
				}
			}
			if (SearchObjectIndex[SearchObjectSlot[Index]] == INDEX_NONE)
				SearchObjectSlot[Index] = 0;
		}
	}
}
